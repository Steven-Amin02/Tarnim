    ...
    public class PowerPointService
    {
        ...
        public PowerPointResult OpenSongPresentation(int songNumber)
        {
            try
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[PowerPointService] File not found: {filePath}");
                    // List available files for debugging
                    var availableFiles = Directory.GetFiles(_pptxFolderPath, "*.pptx");
                    string fileList = string.Join("\n", availableFiles.Select(Path.GetFileName));
                    return new PowerPointResult
                    {
                        Success = false,
                        ErrorMessage = $"ملف العرض غير موجود:\n{fileName}\n\nالملفات المتاحة:\n{(fileList != "" ? fileList : "لا توجد ملفات")}"
                    };
                }
